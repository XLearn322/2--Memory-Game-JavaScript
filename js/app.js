 ...
 *   - (doneasdfasdfasdf) loop through each card and create its HTML
 *   - (done) add each carfasdfasdfasdfasd's HTML to the page
 */
...
function displayCardSymbol() {
  this.setAttribute(
    "style",
    "transition: transform .9s; transform-style: preserve-3d;"
  );
...
function winningMessage() {
  winningCard.setAttribute(
    "style",
    "display:block;box-shadow: 0 4px 10px rgba(0, 0, 0, 0.5);width: 680px;height: 680px;"
  ); //show winning cards
  ...
  switch (starContainer.childElementCount) {
    ...
    case 3:
      ...
      break;
    case 0:
      timeHeader.insertAdjacentHTML("afterbegin", `<h3>Congratulation</h3>`);
      break;
  }
